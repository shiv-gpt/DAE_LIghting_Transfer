...
class FareMultipieLightingTripletsFrontal(data.Dataset):
    ...
    def __init__(self, opt, root, root_mask, 
        resize = 64,
        transform=None, return_paths=False, generate_req_data = False,
        save_data_path= "/nfs/bigdisk/bsonawane/multipie_lighting_transfer_dataset/"):
        self.opt = opt
        ...
        # print(img_map)
        # self.save_images()
        self.generate_req_data = generate_req_data
        self.save_data_path = save_data_path
        if self.generate_req_data:
            self.data_set = self.convert_one_to_one()
            self.save_data()
        self.source_image_list, self.dest_image_list = self.list_dir()
    ...
    def save_data(self):
        ...
        source_path = os.path.join(self.save_data_path, "source")
        dest_path = os.path.join(self.save_data_path, "destination")
        os.mkdir(source_path)
        os.mkdir(dest_path)
        for i in range(len(self.data_set)):
            imsave(source_path + "/" + str(i) + "_" + str(self.data_set[i][0]) + ".jpg", self.data_set[i][1])
            imsave(dest_path + "/" + str(i) + "_" + str(self.data_set[i][2]) + ".jpg", self.data_set[i][3])
            if i % 1000 == 0:
                print(i)
        print("Data Saved to " + self.save_data_path)
    ...
    def list_dir(self):
        ...
        if os.path.isdir(source_path):
            source_images_list = os.listdir(source_path)
        if os.path.isdir(dest_path):
            dest_images_list = os.listdir(dest_path)
        if len(source_images_list) > 0 and len(source_images_list) == len(dest_images_list):
            return source_images_list, dest_images_list
        else:
            raise ValueError("Problem with Dataset!!")
    ...
    def __getitem__(self, index):
        source_path = os.path.join(self.save_data_path, "source")
        dest_path = os.path.join(self.save_data_path, "destination")
        if index < len(self.source_image_list):
            src_image = Image.open(os.path.join(source_path, self.source_image_list[index]))
            src_light = int(self.source_image_list[index].split(".")[0].split("_")[1])
            dest_image = Image.open(os.path.join(dest_path, self.dest_image_list[index]))
            dest_light = int(self.dest_image_list[index].split(".")[0].split("_")[1])
            if self.transform:
                src_image = self.transform(src_image)
                dest_image = self.transform(dest_image)
            return src_light, src_image, dest_light, dest_image
        return None, None, None, None

    def __len__(self):
        return len(self.source_image_list)
