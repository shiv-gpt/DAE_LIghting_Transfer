...
if ON_SERVER:
    out_path = '/nfs/bigdisk/bsonawane/LightTransfer_Exp/Latent_Space/6_CNN_64_DL_test/'    #cropped_multipie/4_manual_masking_map_approach_for_lighting_only_multipie_cropped_63/'
...
TrainingData = []
TrainingData.append(opt.dirDataroot + 'small_train/001')
...
TestingData.append(opt.dirDataroot + 'small_test/008')
...
if opt.use_celeba:
    ...
    dataset = lightDL.FareMultipieLightingTripletsFrontal(None, root=TrainingData, root_mask=TrainingMask, transform = None, resize = 64, generate_req_data = True, save_data_path= "/nfs/bigdisk/bsonawane/multipie_lighting_transfer_dataset/small_train")
    # train_amount = train_amount + len(dataset)
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=opt.batchSize, shuffle=True, num_workers=int(opt.workers))


    dataset_test = lightDL.FareMultipieLightingTripletsFrontal(None, root=TestingData, root_mask = TestingMask, transform = None, resize=64, generate_req_data = True, save_data_path= "/nfs/bigdisk/bsonawane/multipie_lighting_transfer_dataset/small_test")
