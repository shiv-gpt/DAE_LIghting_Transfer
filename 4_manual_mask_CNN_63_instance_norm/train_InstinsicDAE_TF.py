...
if ON_SERVER:
    out_path = './results_CNN_Latent_64_BN/'  # '/nfs/bigdisk/bsonawane/LightTransfer_Exp/Latent_Space/2_CNN_64/'    #cropped_multipie/4_manual_masking_map_approach_for_lighting_only_multipie_cropped_63/'
...
for epoch in range(opt.epoch_iter):
        ...
        for batch_idx, data_point in enumerate(dataloader, 0):
            ...
            #pdb.set_trace()
            loss_encdec = loss_recon.item() + loss_br.item() + loss_tvw.item() + loss_intr_S.item()

            train_loss += loss_encdec

            iter_mark+=1
            print('Iteration[%d] loss -- all:  %.4f .. recon:  %.4f .. tvw: %.4f .. br: %.4f .. intr_s: %.4f .. '
                % (iter_mark,  loss_encdec, loss_recon.item(), loss_tvw.item(), loss_br.item(), loss_intr_S.item()))
