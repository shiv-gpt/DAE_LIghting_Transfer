...
class LightingTransfer(nn.Module):
    ...
    def fill_illlumination_map(self):
        ...
        #illumination_map[25] = generate_random()
        #illumination_map[26] = generate_random([0.7, 0.3])
        #illumination_map[27] = generate_random([0.001, 0.999])
        #illumination_map[28] = generate_random([0.9, 0.1])
        #illumination_map[29] = generate_random([0.4, 0.6])
        #illumination_map[30] = generate_circle(2)
        illumination_map[25] = generate_circle(5)
        illumination_map[26] = generate_circle(20)
        #illumination_map[33] = generate_circle(15)
        illumination_map[27] = generate_circle(30)
        #illumination_map[35] = generate_inv_circle(2)
        #illumination_map[36] = generate_inv_circle(5)
        #illumination_map[37] = generate_inv_circle(10)
        #illumination_map[38] = generate_inv_circle(15)
        #illumination_map[39] = generate_inv_circle(20)
        self.illumination_map = torch.tensor(illumination_map).type(torch.cuda.FloatTensor)
        b_size, h, w          = self.illumination_map.shape
        self.illumination_map = torch.reshape(self.illumination_map, (b_size, 1, h, w))
...
def generate_random(P=[0.3, 0.7]):
    l = [[0]*63 for i in range(63)]
    for i in range(63):
        for j in range(63):
            l[i][j] = np.random.choice([0,1], 1, p=P)[0]
    return l
