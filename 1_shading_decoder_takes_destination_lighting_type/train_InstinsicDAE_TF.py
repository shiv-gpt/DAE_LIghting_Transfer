...
if ON_SERVER:
    out_path  = './results_instance_norm/'   # '/nfs/bigdisk/bsonawane/dae-exp1-dest-in-shader-decoder_mask_all'
...
dataset = lightDL.FareMultipieLightingTripletsFrontal(None, root=TrainingData, root_mask = [], transform = None, resize=64)
# print('# size of the current (sub)dataset is %d' %len(dataset))
# train_amount = train_amount + len(dataset)
dataloader = torch.utils.data.DataLoader(dataset, batch_size=opt.batchSize, shuffle=True, num_workers=int(opt.workers))


dataset_test = lightDL.FareMultipieLightingTripletsFrontal(None, root=TestingData, root_mask = [], transform = None, resize=64)
